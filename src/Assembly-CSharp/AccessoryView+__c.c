...

{
  ...
  pIVar2 = (Il2CppRuntimeInterfaceOffsetPair *)0x1;
  uVar3 = 0;
  pIVar4 = x->klass;
...
code_?:
  ...
  (*pVVar6->methodPtr)(x,1,pVVar6->method,pVVar6->methodPtr);
  return;
}
...
{
  if (cRam_? == '\0') {
    FUN_?(&TypeInfo__UnityEngine__EventSystems__IAccessoryInventoryControl);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
    ...
    if (uVar3 != 0) {
      do {
        if (pIVar2->interfaceOffsets[uVar1].interfaceType ==
          ...
          pVVar4 = &(pIVar2->vtable).DisplayPurchasableItems +
                   (pIVar2->interfaceOffsets[uVar1].offset + 3);
...

{
  ...
  pAVar1 = (AccessoryView_c *)FUN_?(TypeInfo__AccessoryView____c);
  TypeInfo__AccessoryView____c->static_fields->__9 = pAVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__AccessoryView____c->static_fields >> 0xc);
