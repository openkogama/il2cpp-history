...

{
  ...
  if (pLVar9 != (LimbController *)0x0) {
    if (_UNK_? <
        fVar6 * (pLVar9->fields).interpolateTowardsPitchRotation.y +
...

{
  ...
  if (pLVar2 != (LimbController *)0x0) {
    if (_UNK_? <
        pitchRotation.y * (pLVar2->fields).interpolateTowardsPitchRotation.y +
        pitchRotation.x * (pLVar2->fields).interpolateTowardsPitchRotation.x +
        pitchRotation.z * (pLVar2->fields).interpolateTowardsPitchRotation.z +
