    ...
    if (uVar3 != 0) {
      do {
        if (pIVar2->interfaceOffsets[uVar1].interfaceType ==
          ...
          (*UNRECOVERED_JUMPTABLE)(z,pVVar4->method,UNRECOVERED_JUMPTABLE);
          return;
        }
        uVar1 = uVar1 + 1;
      } while (uVar1 < uVar3);
    }
    puVar5 = (undefined8 *)FUN_?(z);
                    /* WARNING: Could not recover jumptable at 0xADDR. Too many branches */
                    /* WARNING: Treating indirect jump as call */
    (*(code *)*puVar5)(z,puVar5[1],(code *)*puVar5);
    return;
  }
  FUN_?();
  pcVar6 = (code *)swi(3);
...

{
  ...
  pVVar5 = (VirtualInvokeData *)FUN_?(z);
code_?:
                    /* WARNING: Could not recover jumptable at 0xADDR. Too many branches */
                    /* WARNING: Treating indirect jump as call */
  (*pVVar5->methodPtr)(z,6,pVVar5->method,pVVar5->methodPtr);
  return;
}
...
{
  if (cRam_? == '\0') {
    FUN_?(&TypeInfo__ReviveUIHandlerBase____c);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
  pRVar1 = (ReviveUIHandlerBase_c *)FUN_?(TypeInfo__ReviveUIHandlerBase____c);
  TypeInfo__ReviveUIHandlerBase____c->static_fields->__9 = pRVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__ReviveUIHandlerBase____c->static_fields >> 0xc);
