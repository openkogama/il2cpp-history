...

{
  ...
  if (eventData != (PointerEventData *)0x0) {
    VStackX_8.y = (eventData->fields)._position_k__BackingField.y - (this->fields).startPos.y;
    VStackX_8.x = (eventData->fields)._position_k__BackingField.x - (this->fields).startPos.x;
    fVar1 = (float)FUN_?(&VStackX_8);
    if (cRam_? == '\0') {
    ...
    VStackX_8 = ResolutionManager::ResolutionManager_get_PixelsToPhysicalDistance((MethodInfo *)0x0)
    ;
    if (fVar1 <= VStackX_8.x * _UNK_?) {
      return;
    }
    pTVar2 = (this->fields).touchPadAbsolute;
    if (pTVar2 != (TouchPadAbsolute *)0x0) {
      uVar3 = (uint)((this->fields).horizontalEnabled != 0);
      uVar4 = uVar3 | 2;
      if ((this->fields).verticalEnabled == 0) {
        uVar4 = uVar3;
      }
      (pTVar2->fields).state = uVar4;
      pTVar2 = (this->fields).touchPadAbsolute;
      if (pTVar2 != (TouchPadAbsolute *)0x0) {
        if ((pTVar2->fields).dragging == 0) {
          (pTVar2->fields).dragging = 1;
          UnityStandardAssets::CrossPlatformInput::TouchPadAbsolute::
          TouchPadAbsolute_SetOnPointButton(pTVar2,1,(MethodInfo *)0x0);
          (pTVar2->fields).fingerId = (eventData->fields)._pointerId_k__BackingField;
          fVar1 = (eventData->fields)._position_k__BackingField.x;
          (pTVar2->fields).previousInputPosition.y = (eventData->fields)._position_k__BackingField.y
          ;
          (pTVar2->fields).previousInputPosition.x = fVar1;
        }
