    ...
    if (uVar3 != 0) {
      do {
        if (pIVar2->interfaceOffsets[uVar1].interfaceType ==
          ...
          pVVar4 = &(pIVar2->vtable).DeleteWoid + (pIVar2->interfaceOffsets[uVar1].offset + 1);
...

{
  ...
  pPVar1 = (PMOWalkMode_c *)FUN_?(TypeInfo__PMOWalkMode____c);
  TypeInfo__PMOWalkMode____c->static_fields->__9 = pPVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__PMOWalkMode____c->static_fields >> 0xc);
