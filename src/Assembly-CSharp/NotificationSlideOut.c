...
{
  if (cRam_? == '\0') {
    FUN_?(&Notification_MethodInfo__UnityEngine__Component__GetComponent<Notification>__);
    LOCK();
    UNLOCK();
  ...
  if (bVar2) {
    uVar3 = (uint)((ulonglong)&(this->fields).notification >> 0xc);
...
code_?:
  ...
  fVar3 = (pNVar1->fields).timeSinceStart;
  iVar4 = (*(pNVar1->klass->vtable).__unknown.methodPtr)();
  fVar3 = fVar3 / (float)iVar4;
  pfVar5 = &(this->fields).slideOutStartTime;
  if (*pfVar5 <= fVar3 && fVar3 != *pfVar5) {
    ...
    fVar3 = (this->fields).slideVelocity.x;
    ...
    fStackX_8 = VVar7.x;
    fStackX_c = VVar7.y;
    VVar7.y = fVar8 * fVar10 + fStackX_c;
    VVar7.x = fVar3 * fVar10 + fStackX_8;
