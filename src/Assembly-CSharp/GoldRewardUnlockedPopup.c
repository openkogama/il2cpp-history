...

{
  ...
  pcRam_? = pcVar1;
  fVar3 = (float)(*pcRam_?)();
  pTVar4 = (this->fields).titleText;
  (this->fields).bounceEffectStartTime = fVar3;
  pSVar5 = TM::TM__((this->fields).titleString,(MethodInfo *)0x0);
  auStackX_18[0] = 2;
...

{
  ...
  if (obj != (AnimationCurve *)0x0) {
    ...
    obj_00 = (this->fields).imageContentTransform;
...
{
  if (cRam_? == '\0') {
    FUN_?(&StringLiteral__0__GOLD_CLAIMED_);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
  ...
  if (bVar1) {
    uVar2 = (uint)((ulonglong)&(this->fields).titleString >> 0xc);
