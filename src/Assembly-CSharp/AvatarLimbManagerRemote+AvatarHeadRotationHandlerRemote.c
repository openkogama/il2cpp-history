...

{
  ...
  if (lookDirectionHandler != (AvatarLimbManager_AvatarLookDirectionHandler *)0x0) {
    ...
    if (pNVar6 != (NavMesh_OnNavMeshPreUpdate *)0x0) {
      ...
      limbRotator = (AvatarLimbManager_LimbRotator *)0x0;
      if (pAVar5 == (Action *)0x0) {
        (lookDirectionHandler->fields).OnRotationChange = (Action *)0x0;
      }
      else {
        if (pAVar5->klass == TypeInfo__System__Action) {
          limbRotator = (AvatarLimbManager_LimbRotator *)pAVar5;
        }
        if ((Action *)limbRotator == (Action *)0x0) goto code_?;
        (lookDirectionHandler->fields).OnRotationChange = (Action *)limbRotator;
        limbRotator = (AvatarLimbManager_LimbRotator *)(Action *)0x0;
        if (pAVar5->klass == TypeInfo__System__Action) {
          limbRotator = (AvatarLimbManager_LimbRotator *)pAVar5;
        }
        if ((Action *)limbRotator == (Action *)0x0) goto code_?;
      ...
      func_?();
      if (cRam_? == '\0') {
        limbRotator = (AvatarLimbManager_LimbRotator *)&TypeInfo__System__Action;
        func_?();
        func_?();
        cRam_? = '\x01';
      }
