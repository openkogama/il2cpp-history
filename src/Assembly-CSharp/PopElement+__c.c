...
{
  if (cRam_? == '\0') {
    FUN_?(&TypeInfo__PopElement____c);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
  pPVar1 = (PopElement_c *)FUN_?(TypeInfo__PopElement____c);
  TypeInfo__PopElement____c->static_fields->__9 = pPVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__PopElement____c->static_fields >> 0xc);
