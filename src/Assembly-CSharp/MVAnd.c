...

{
  if (cRam_? == '\0') {
    ...
    LOCK();
    UNLOCK();
    FUN_?(&MethodInfo__MVAnd__SignalCallback_bool__bool__LogicObjectManager_);
    LOCK();
    UNLOCK();
...
code_?:
    (*(code *)*puVar27)(plVar26,b);
    *(bool *)((longlong)&pOVar15[1].monitor + 4) = b;
  }
  FUN_?();
code_?:
  FUN_?();
...

{
  ...
  if (pPVar1 != (PrefabPool *)0x0) {
    MVLogicObject::MVLogicObject__ctor
              ((MVLogicObject *)this,data,(pPVar1->fields).mvAndPrefab,worldObjects,
               (MethodInfo *)0x0);
