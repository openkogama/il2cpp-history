...

{
  ...
  if (iVar2 == 0) {
    ...
    fVar6 = (float10)(*(pLVar5->klass->vtable).get_minHeight_1.methodPtr)
                               (pLVar5,(pLVar5->klass->vtable).get_minHeight_1.method);
  ...
  if ((pTVar3->fields).lerpTime < (float)this - (pTVar3->fields).startTime) {
    if (pLVar5 != (LayoutElement *)0x0) {
      (*(pLVar5->klass->vtable).set_minHeight.methodPtr)
                (pLVar5,(fStack_8 - fStack_7) + fStack_7,
                 (pLVar5->klass->vtable).set_minHeight.method);
      return 0;
    }
  }
  else {
