...
{
  if (cRam_? == '\0') {
    FUN_?(&TypeInfo__UnityEngine__EventSystems__IBundleController);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
    ...
    if (uVar3 != 0) {
      do {
        if (pIVar2->interfaceOffsets[uVar1].interfaceType ==
          ...
          pVVar4 = &(pIVar2->vtable).ShowBundle + (pIVar2->interfaceOffsets[uVar1].offset + 1);
          UNRECOVERED_JUMPTABLE = pVVar4->methodPtr;
                    /* WARNING: Could not recover jumptable at 0xADDR. Too many branches */
                    /* WARNING: Treating indirect jump as call */
          (*UNRECOVERED_JUMPTABLE)(x,pVVar4->method,UNRECOVERED_JUMPTABLE);
...

{
  ...
  pBVar1 = (BundleView_c *)FUN_?(TypeInfo__BundleView____c);
  TypeInfo__BundleView____c->static_fields->__9 = pBVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__BundleView____c->static_fields >> 0xc);
