...
{
  if (cRam_? == '\0') {
    FUN_?(&TypeInfo__ESCubeEdit____c);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
  pEVar1 = (ESCubeEdit_c *)FUN_?(TypeInfo__ESCubeEdit____c);
  TypeInfo__ESCubeEdit____c->static_fields->__9 = pEVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__ESCubeEdit____c->static_fields >> 0xc);
