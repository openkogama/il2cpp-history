    ...
    if (uVar3 != 0) {
      do {
        if (pIVar2->interfaceOffsets[uVar1].interfaceType ==
          ...
          pVVar4 = &(pIVar2->vtable).Open + (pIVar2->interfaceOffsets[uVar1].offset + 1);
...

{
  ...
  pEVar1 = (ExitCubeModeling_c *)FUN_?(TypeInfo__ExitCubeModeling____c);
  TypeInfo__ExitCubeModeling____c->static_fields->__9 = pEVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__ExitCubeModeling____c->static_fields >> 0xc);
