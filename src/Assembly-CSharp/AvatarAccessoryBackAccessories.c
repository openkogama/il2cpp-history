...

{
  if (cRam_? == '\0') {
    ...
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_WingsAttachPoint);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
...

{
  ...
  if (pAVar1 != (AccessoryBackAccessoriesSettings *)0x0) {
    if (*(int *)&(TypeInfo__UnityEngine__Object->_1).field_0x1c == 0) {
      FUN_?();
    }
  ...
  bVar3 = iRam_? == 0;
  (this->fields).wingSettings = (AccessoryBackAccessoriesSettings *)pAVar2;
  if (bVar3) {
    return pAVar2;
  }
  uVar4 = (uint)((ulonglong)&(this->fields).wingSettings >> 0xc);
