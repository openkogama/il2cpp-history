...

{
  if (cRam_? == '\0') {
    ...
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_BUNDLE);
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_PARTICLES);
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_LEVEL_UNLOCKS);
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_BACK_ACCESSORIES);
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_HATS);
    LOCK();
    UNLOCK();
    FUN_?(&StringLiteral_FEATURED);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
  ...
  if (map != (Dictionary_2_System_Int32_System_String_ *)0x0) {
    ...
    mscorlib.dll::System::Collections::Generic::Dictionary`2[System::Int32,System::Object]::
    Dictionary_2_System_Int32_System_Object__TryInsert
              ((Dictionary_2_System_Int32_System_Object_ *)map,0xfe,(Object *)pSVar1,IVar2,
               MethodInfo__System__Collections__Generic__Dictionary<int,_System::String>__Add_int__System__String_
               ->klass->rgctx_data[0x22].method);
...

{
  ...
  if (bVar1) {
    uVar2 = (uint)((ulonglong)TypeInfo__LocalizedEnums__AccessoryCategoryLS->static_fields >> 0xc);
