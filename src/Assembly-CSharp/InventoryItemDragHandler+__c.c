    ...
    if (uVar3 != 0) {
      do {
        if (pIVar2->interfaceOffsets[uVar1].interfaceType ==
          ...
          pVVar4 = &(pIVar2->vtable).Activate + (pIVar2->interfaceOffsets[uVar1].offset + 6);
...

{
  ...
  pIVar1 = (InventoryItemDragHandler_c *)FUN_?(TypeInfo__InventoryItemDragHandler____c);
  TypeInfo__InventoryItemDragHandler____c->static_fields->__9 = pIVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__InventoryItemDragHandler____c->static_fields >> 0xc);
