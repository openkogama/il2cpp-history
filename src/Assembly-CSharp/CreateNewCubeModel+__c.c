...
code_?:
  ...
  (*pVVar5->methodPtr)(handler,0x1e,pVVar5->method,pVVar5->methodPtr);
  return;
}
...
{
  if (cRam_? == '\0') {
    FUN_?(&TypeInfo__CreateNewCubeModel____c);
    LOCK();
    UNLOCK();
    cRam_? = '\x01';
  }
  pCVar1 = (CreateNewCubeModel_c *)FUN_?(TypeInfo__CreateNewCubeModel____c);
  TypeInfo__CreateNewCubeModel____c->static_fields->__9 = pCVar1;
  if (iRam_? != 0) {
    uVar2 = (uint)((ulonglong)TypeInfo__CreateNewCubeModel____c->static_fields >> 0xc);
